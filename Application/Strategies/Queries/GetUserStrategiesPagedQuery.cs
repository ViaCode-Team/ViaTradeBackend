...
{
	public async Task<PagedResult<UserTradeStrategy>> Handle(GetUserStrategiesPagedQuery request, CancellationToken ct)
	{
		return await userTradeStrategyRepository.GetByUserPagedAsync(request.UserId, request.PaginationRequest, ct);
	}
}
