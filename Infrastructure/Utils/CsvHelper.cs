...
//            {
//                [TradeDataType.Futures] = Path.Combine(basePath, _options.FuturesDataDirectoryName),
//                [TradeDataType.Stocks] = Path.Combine(basePath, _options.StocksDataDirectoryName),
//                [TradeDataType.Strategy] = Path.Combine(basePath, _options.StrategyResultDirectoryName),
//                [TradeDataType.Screener] = Path.Combine(basePath, _options.ScrennerResultDirectoryName)
//            };
//        }

//        public IEnumerable<string> GetFileNames(TradeDataType dataType)
//        {
//            return _fileReader.GetFileNames(dataType);
//        }

//        public async Task<Dictionary<TradeDataType, List<string>>> GetAllFilesAsync(CancellationToken ct = default)
//        {
//            await Task.Yield();

//            return Enum.GetValues<TradeDataType>().ToDictionary(
//                type => type,
...
//        public IEnumerable<T> ReadData<T>(TradeDataType dataType, string fileName, DateTime? startDate = null, DateTime? endDate = null) where T : class
//        {
...
//                try
//                {
//                    var results = _fileReader.ReadData<StrategyResult>(TradeDataType.Strategy, fileName, startDate, endDate);
...
//                try
//                {
//                    var data = _fileReader.ReadData<ScreenerData>(TradeDataType.Screener, fileName, startDate, endDate);
...
//            var futuresFiles = _fileReader.GetFileNames(TradeDataType.Futures)
//                .Where(f => f.StartsWith(instrumentCode, StringComparison.OrdinalIgnoreCase));

//            if (futuresFiles.Any())
//            {
//                result.DataType = TradeDataType.Futures;
//                result.Bars = LoadTradingData(TradeDataType.Futures, futuresFiles, startDate, endDate, ct);
//            }
//            else
//            {
//                var stocksFiles = _fileReader.GetFileNames(TradeDataType.Stocks)
//                    .Where(f => f.StartsWith(instrumentCode, StringComparison.OrdinalIgnoreCase));

//                if (stocksFiles.Any())
//                {
//                    result.DataType = TradeDataType.Stocks;
//                    result.Bars = LoadTradingData(TradeDataType.Stocks, stocksFiles, startDate, endDate, ct);
//                }
//            }

//            var strategyFiles = _fileReader.GetFileNames(TradeDataType.Strategy)
//                .Where(f => f.StartsWith(instrumentCode, StringComparison.OrdinalIgnoreCase));
...
//            var screenerFiles = _fileReader.GetFileNames(TradeDataType.Screener)
//                .Where(f => f.StartsWith(instrumentCode, StringComparison.OrdinalIgnoreCase));
...
//                try
//                {
//                    var results = _fileReader.ReadData<StrategyResult>(TradeDataType.Strategy, fileName);
